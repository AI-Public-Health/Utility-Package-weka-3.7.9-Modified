...
//        for (int i=0; i<bayesNet.m_Distributions.length; i++){
//        	for (int j=0; j<bayesNet.m_Distributions[i].length; j++){
//        		System.out.print(bayesNet.m_Distributions[i][j]+",");
//        	}
//        }
//        System.out.println();
    } // updateClassifier
            ...
            for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
                ...
                for (int iParent = 0; iParent < bayesNet.getParentSet(iAttribute).getNrOfParents(); iParent++) {
                    ...
                    if (nParent == instances.classIndex()) {
                    	...
                    	//Ye added begin
                    	String name_nParent = bayesNet.getNodeName(nParent);
                    	int tempIndex = -1;
                    	for (int index=0; index<instance.numAttributes(); index++){
                    		if (instance.attribute(index).name().equals(name_nParent)){
                    			tempIndex = index;
                    			break;
                    		}
                    	}
                    	iCPT = iCPT * instances.attribute(nParent).numValues() + instance.value(tempIndex);
                ...
                if (iAttribute == instances.classIndex()) {
                	...
                	//Ye added begin
                	String name_iAttribute = bayesNet.getNodeName(iAttribute);
                	int tempIndex = -1;
                	for (int index=0; index<instance.numAttributes(); index++){
                	 ...
                	 logfP
                           += Math.log(bayesNet.m_Distributions[iAttribute][(int) iCPT].getProbability(instance.value(tempIndex)));
                	//end Ye added
//                    logfP
